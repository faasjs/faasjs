...
FaasJS 应用切片是一个小型、完整的垂直功能，它将 UI、API、验证、数据库变更、CLI 工具、任务、类型和测试集中在一起，使其易于查找和审查。切片通过为人类和代理提供可直接遵循的稳定约定，取代了以生成器为中心的工作流。

## 切片包含的内容

一个数据库驱动的业务切片通常包括：

- `src/features/<feature>/` 下的功能文件夹
- 可选的 React 入口，如 `src/features/<feature>/index.tsx`
- 归属于该功能的组件、hooks、服务、schema、CLI 工具或任务
- 当功能暴露后端 action 时，放在该功能 `api/` 目录下的一个或多个 `.api.ts` 文件
...
`create-faas-app` 生成的 `admin` 启动模板包含一个可复制的 users 切片。对于 users 切片，推荐使用 feature based 布局：
...
对于嵌套能力，将它们保留在拥有它们的功能下，而不是移动到脱离业务语义的路由、API 或工具树中：
...
在切片内使用短相对导入，除非已配置现有的 TypeScript 别名。注意 `.api.ts` 和 `.job.ts` 的路径具有外部语义：`src/features/users/api/list.api.ts` 映射到 `/features/users/api/list`，`src/features/users/jobs/sync.job.ts` 映射到 `features/users/jobs/sync`。
...
保持功能 UI 的状态和请求流可读。在创建、更新或删除流程之后，有意刷新、关闭或失效受影响的界面并显示用户反馈。仅在组件被复用、创建了真正的边界或简化了大块代码时，才提取组件、hooks 或辅助函数。

## 测试

将 API 测试放在 API 文件夹的 `__tests__` 下。

对于非 API 测试，使用相同的功能本地布局原则：将 React、任务、CLI、辅助或集成测试放在它们所覆盖的功能、任务、工具或切片文件夹内的 `__tests__` 文件夹中，而不是集中到独立的包级测试树中。
