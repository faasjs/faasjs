...
在实现或审查 FaasJS 应用中的标准 CRUD 功能（列表、详情、创建、更新、删除）时使用本指南。它涵盖了从 API 端点到 React 页面的完整垂直切片。

有关更深入的规则，请参考[应用切片指南](./application-slices.md)、[Ant Design 指南](./ant-design.md)、[defineApi 指南](./define-api.md)和 [React 数据获取指南](./react-data-fetching.md)。本指南专注于将这些模式组合成完整的 CRUD 循环。

## 默认工作流

1. 将业务字段建模为共享的 `items` 元数据，放在 `use<Feature>Items` hook 中。
2. 创建五个 API 文件：`list`、`detail`、`create`、`update`、`remove`。
3. 使用 `Table.faasData`、搜索/筛选控件和操作列构建列表页面。
4. 使用抽屉内的 `Description.faasData` 添加详情视图。
5. 添加复用相同 `items` 并使用 `Form.faas` 的创建/更新表单。
6. 使用模态框确认添加删除功能。
7. 连接变更反馈（`message.success`、`notification.error`）和界面刷新/关闭。
8. 使用 `testApi` 添加 API 测试，覆盖成功、验证和错误路径。
9. 使用 `setMock` 添加 React 测试，覆盖加载、渲染和变更流程。
...
| 操作 | API 文件            | 路由                                  | 方法 | 参数                |
| ---- | ------------------- | ------------------------------------- | ---- | ------------------- |
| 列表 | `api/list.api.ts`   | `POST /features/<feature>/api/list`   | post | 筛选、分页          |
| 详情 | `api/detail.api.ts` | `POST /features/<feature>/api/detail` | post | `{ id }`            |
| 创建 | `api/create.api.ts` | `POST /features/<feature>/api/create` | post | 业务字段            |
| 更新 | `api/update.api.ts` | `POST /features/<feature>/api/update` | post | `{ id, ...fields }` |
| 删除 | `api/remove.api.ts` | `POST /features/<feature>/api/remove` | post | `{ id }`            |

### 文件命名约定

| 层       | 约定                                         | 示例                                        |
| -------- | -------------------------------------------- | ------------------------------------------- |
| 功能 UI  | `features/<feature>/index.tsx`               | `features/users/index.tsx`                  |
| 组件     | `features/<feature>/components/*.tsx`        | `features/users/components/UserTable.tsx`   |
| Hooks    | `features/<feature>/hooks/*.ts`              | `features/users/hooks/useUserItems.ts`      |
| API      | `features/<feature>/api/*.api.ts`            | `features/users/api/list.api.ts`            |
| API 测试 | `features/<feature>/api/__tests__/*.test.ts` | `features/users/api/__tests__/list.test.ts` |

## 共享 Items 模式

`items` 数组是跨 `Form`、`Table` 和 `Description` 的业务字段的单一事实来源。每个 item 描述一个字段：其 id、标签、输入类型、验证和可选的特定界面渲染器。

在 `use<Feature>Items` hook 中定义 items，使它们保持同处一处且可复用。
...
- 将 items 保持在 `useConstant` 中，使数组引用在渲染间保持稳定。
- 仅当呈现确实因界面而异时，才添加 `tableRender`、`descriptionRender` 或 `formRender`。
- 对于仅出现在一个界面上的字段（如操作列），在消费组件上内联添加，而不是污染共享 items。
- 使用 `extendTypes` 处理重复的自定义字段行为（参见 [Ant Design 指南](./ant-design.md#核心模式)）。

## 列表页面模式

使用 `Table.faasData` 进行服务端驱动的列表获取。在表格上方添加搜索/筛选控件行，以及用于详情/编辑/删除的操作列。
...
- 使用 `setSearchParams` 触发 `Table.faasData` 重新获取。当 `faasData.params` 改变时，表格会自动重新获取。
- 通过 `useFaas` 生命周期使用 `debounce`，而不是自定义定时器（参见 [React 数据获取指南](./react-data-fetching.md)）。
- 将 `keyword` 保留为输入的本地状态，仅在提交搜索时推送到 `searchParams`。

## 详情模式

使用 `Description.faasData` 进行详情获取。在抽屉中打开以保持列表上下文可见。
...
- 复用 `use<Feature>Items` 中的相同 `items`。
- 对于在详情中不同的字段，在 item 上使用 `descriptionRender`。
- 保持详情 API 响应形状与 item 字段 id 一致。

## 创建表单模式

使用 `Form.faas` 进行表单提交，内置加载、验证、反馈和错误处理。
...
- `Form.faas` 自动处理按钮加载、验证和错误反馈。
- 使用 `message.success` 提供成功反馈，使用 `notification.error` 提供失败反馈。
- 成功时通过 `setDrawerProps({ open: false })` 关闭抽屉/模态框。
- 调用 `props.onSuccess?.()` 以便父组件可以刷新列表。

## 更新表单模式

通过传递 `initialValues` 并切换 API action 来复用相同的 `UserForm` 组件。表单从详情获取中预填充数据。
...
- 对创建和更新使用相同的 `UserForm` 组件——`id` 属性决定模式。
- 在打开抽屉的页面/组件上获取当前值，然后将其作为 `initialValues` 传入。
- 使更新 API 字段可选（`z.string().optional()`），以支持部分更新。
- 使用 `Form.faas` 的 `params` 函数有条件地为更新包含 `id`。

## 删除模式

对破坏性操作使用模态框确认。在 `onOk` 处理程序中命令式地调用 `faas`。
...
- 使用 `useApp()` 的 `setModalProps` 进行模态框确认。
- 在 `onOk` 处理程序内部命令式地调用 `faas`——而不是 `Form.faas`。
- 始终先获取记录，如果未找到则返回 `404`。
- 成功时，关闭模态框，显示 `message.success`，并调用 `onSuccess` 刷新父组件。
...
```
src/features/users/
  index.tsx                          # 功能 UI 入口：组合 UserTable、创建按钮
  components/
    UserTable.tsx                    # 带有 faasData、搜索、操作列的表格
    UserForm.tsx                     # 用于创建/更新的表单（id 属性切换模式）
    UserDetail.tsx                   # 带有 faasData 的 Description
    UserActions.tsx                  # 详情/编辑/删除按钮
  hooks/
    useUserItems.ts                  # 共享的 items 元数据
  api/
...
| 文件              | 行数（约） | 职责                           |
| ----------------- | ---------- | ------------------------------ |
| `index.tsx`       | 30-60      | 功能 UI 入口、布局、组合组件   |
| `UserTable.tsx`   | 40-80      | 表格、搜索、筛选、操作列       |
| `UserForm.tsx`    | 30-60      | 表单项、faas 配置、反馈        |
| `UserDetail.tsx`  | 15-30      | 带有 faasData 的 Description   |
| `UserActions.tsx` | 40-70      | 详情/编辑/删除按钮、模态框确认 |
| `useUserItems.ts` | 30-60      | 共享的 items 元数据            |
| 每个 `.api.ts`    | 15-40      | Schema、处理程序、数据库查询   |
| 每个 API 测试     | 20-50      | testApi、成功/错误路径         |

## 测试 CRUD 端点

使用 `@faasjs/dev` 的 `testApi` 测试每个端点。覆盖成功、验证失败和预期的业务错误。
...
- 对每个端点测试：成功路径、验证失败（400）和业务错误（404、409）。
- 使用 `testApi(api)` 获取类型化处理程序。
- 断言 `statusCode` 和 `data`/`error` 形状。
- 将测试放在 API 文件旁边的 `api/__tests__/` 下。

## 代理效率提示

以下模式可帮助 AI 编码代理 2-3 倍更快地生成完整的 CRUD 功能。
...
Items 是基础。一旦 items 存在，代理可以一致地推导出 `Table`、`Form` 和 `Description`。
...
五个小型的 `.api.ts` 文件一起生成比逐个生成更快，而且代理可以保持 schema 一致。

### 3. 复制粘贴组合表单

带有基于 `id` 的模式切换（创建与更新）的 `UserForm` 模式是可复用的。生成一个组合表单，而不是分开的创建/更新组件。
...
1. 使用共享的 `items` 放在 `use<Feature>Items` hook 中，作为跨 `Form`、`Table` 和 `Description` 的业务字段的单一事实来源。
2. 使用 CRUD 操作命名 API 文件（`list`、`detail`、`create`、`update`、`remove`），并将它们放在 `<feature>/api/` 下。
3. 使用 `Form.faas` 处理创建/更新提交——它自动处理加载、验证和错误反馈。
4. 使用 `Table.faasData` 处理列表获取——它处理加载、错误和参数变化时的重新获取。
5. 使用 `Description.faasData` 处理详情获取。
6. 使用带有 `id` 属性的组合 `UserForm` 组件在创建和更新模式之间切换。
7. 使用 `faas` 命令式地处理删除和其他一次性变更。
8. 每次变更后关闭覆盖层并刷新受影响的界面。
9. 使用 `message.success` 提供成功反馈，使用 `notification.error` 提供失败反馈。
10. 在 detail、update 和 remove 端点中始终先获取资源；未找到时返回 `404`。
11. 在 create/update 端点中检查重复；冲突时返回 `409`。
12. 使用 `@faasjs/dev` 的 `testApi` 将 API 测试放在 `api/__tests__/` 下。
13. 将 `items` 保持在 `useConstant` 中，使数组引用在渲染间保持稳定。
14. 在 `Table` items 上内联添加操作列，而不是放在共享 items 中。
15. 使用 `useApp()` 的 `setDrawerProps` 处理上下文内的创建/编辑/详情覆盖层。
16. 使用 `useApp()` 的 `setModalProps` 处理破坏性确认模态框。

## 审查清单

- [ ] 存在包含所有业务字段的共享 `use<Feature>Items` hook
- [ ] 存在五个 API 文件：`list.api.ts`、`detail.api.ts`、`create.api.ts`、`update.api.ts`、`remove.api.ts`
- [ ] 每个 API 在 `defineApi` 中定义了内联 Zod schema
- [ ] 列表 API 返回 `{ rows, total }` 形状
- [ ] Detail/update/remove API 检查存在性，未找到时返回 `404`
- [ ] Create/update API 检查重复，冲突时返回 `409`
- [ ] `Table.faasData` 驱动列表，带有搜索/筛选参数
- [ ] `Description.faasData` 驱动详情视图
- [ ] `Form.faas` 驱动创建/更新，带有基于 `id` 的模式切换
- [ ] `faas` + 模态框确认驱动删除
- [ ] 变更提供反馈（`message.success`、`notification.error`）
- [ ] 变更关闭覆盖层并调用 `onSuccess` 刷新父组件
- [ ] API 测试覆盖成功路径、400（验证）和业务错误（404/409）
- [ ] API 测试使用 `testApi` 并放在 `api/__tests__/` 中
- [ ] Items 用 `useConstant` 包装
- [ ] 操作列在 `Table` 上内联添加，而非在共享 items 中
- [ ] 创建/编辑/详情使用抽屉；删除确认使用模态框
- [ ] 文件结构遵循 `features/<feature>/{components/,hooks/,api/}` 约定
