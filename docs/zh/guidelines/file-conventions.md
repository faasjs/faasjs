...
在创建或审查功能文件夹、React 组件、hooks、FaasJS 后端路由文件、后台任务文件或功能本地 CLI 工具时，请使用此指南。

> **另请参阅**：[命名约定指南](./naming-convention.md)了解标识符和文件命名规则（camelCase、PascalCase、kebab-case、缩写等）。本指南涵盖文件*放在哪里*；命名规范则说明文件*叫什么*。

## 适用场景

- 创建新的功能、功能 UI、组件或 hooks
- 重构应用目录结构
- 在 `features/` 下添加新的功能目录
- 创建或移动 `.api.ts` 后端文件
- 创建或移动 `.job.ts` 后台任务文件
- 创建或移动功能本地 CLI 或工具文件
...
1. 将业务功能代码放在 `src/features/<feature>/` 下。
2. 当功能暴露 UI 时，使用 `index.tsx` 作为该功能的主 React 入口。
3. 将一次性的功能本地逻辑保持内联，直到有理由提取为止。
4. 当组件或 hook 值得拥有自己的抽象时，为其创建独立文件。
5. 按功能组织 UI、API、任务、CLI 工具、schema 和测试。
6. 在拥有它们的功能内，将组件放在 `components/` 中，hooks 放在 `hooks/` 中，后端路由处理器放在 `api/` 中，后台任务放在 `jobs/` 中，CLI 工具放在 `cli/` 中。
...
- 文件名应该与组件名完全一致。
- 在文件名中保留组件的大小写。
- 功能 UI 入口文件是例外：当文件代表功能入口时，使用 `index.tsx` 或 `default.tsx`。
...
- 业务功能应该放在 `src/features/<feature>/` 下。
- 每个功能应该拥有其本地 UI、API、hooks、任务、CLI 工具、schema 和测试。
- 当文件夹清晰映射到一个功能界面时，功能 UI 入口文件应该使用 `index.tsx`。
- 功能 UI 入口文件应该 `export default` 入口组件。
- 前端路由应该在应用代码或所选的 UI 框架中明确定义。
- 名为 `api/` 的目录保留给后端处理器。
- 组件必须放在 `components/` 下。
- Hooks 必须放在 `hooks/` 下。
- 功能本地后端处理器应该放在该功能的 `api/` 目录下。
- 功能本地后台任务应该放在该功能的 `jobs/` 目录下。
- 功能本地 CLI 工具应该放在该功能的 `cli/` 目录下。
- 只有功能入口文件可以直接放在外层功能目录中。
- 属于同一功能的共享代码应该保留在该功能范围内，而不是平铺在根目录。
- 如果功能文件变得过大而难以舒适浏览，应按照真正的组件、hook、API、任务、CLI 或服务边界进行拆分，而不是创建占位的辅助文件。
...
`src/features` 下的功能组织是项目约定，不是隐式的浏览器路由。在应用或 UI 框架中明确定义浏览器路由。后端 API 路由是独立的，仍然遵循 `src/` 下完整路径的零映射规则。

### 4. 后端文件遵循路由映射

- 后端路由文件必须遵循[路由映射规范](../../../docs/specs/routing-mapping.md)。
- API 入口文件必须以 `.api.ts` 结尾。
- API 文件应该放在拥有它的功能的 `api/` 目录下。
...
- `/features/feature-name/api/*` 回退

### 5. 后台任务放在功能 `jobs/` 下

- 任务入口文件必须以 `.job.ts` 结尾。
- 功能本地任务文件应该放在 `src/features/<feature>/jobs/` 下。
- 跨功能或平台级任务可以放在 `src/jobs/` 下。
...
- 组件文件名与组件名一致
- Hook 文件名与 hook 名一致
- 主功能 UI 入口文件在保持文件夹易于浏览时使用 `index.tsx`
- 功能 UI 入口文件默认导出入口组件
- 业务功能代码放在 `src/features/<feature>/` 下
- 前端组件放在 `components/` 中
- 前端 hooks 放在 `hooks/` 中
- 前端后端处理器放在 `api/` 中
- 功能本地后台任务文件放在 `jobs/` 中并以 `.job.ts` 结尾
- 功能本地 CLI 工具放在 `cli/` 中
- 只有功能入口文件保留在外层功能目录
- 后端 `.api.ts` 文件遵循路由映射
