...
FaasJS already documents public APIs primarily through JSDoc in TypeScript source. [`build-docs.ts`](https://github.com/faasjs/faasjs/blob/main/build-docs.ts) runs TypeDoc against each package entry and emits Markdown under `classes/`, `functions/`, `interfaces/`, `type-aliases/`, and `variables/`.
