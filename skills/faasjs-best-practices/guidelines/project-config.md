...
4. Reuse `oxfmtConfig` and `oxlintConfig` from `@faasjs/dev/configs` instead of duplicating common rules.
...
- Use `@faasjs/types/tsconfig/react` for most FaasJS apps with React pages.
- Use `@faasjs/types/tsconfig/base` for non-React TypeScript apps.
- Use `@faasjs/types/tsconfig/build` for packages, libraries, and workspace modules that are built and published.

React app example:

```json
{
  "extends": "@faasjs/types/tsconfig/react",
  "compilerOptions": {
...
```json
{
  "extends": "@faasjs/types/tsconfig/base",
  "compilerOptions": {
    "types": ["node", "vitest/globals"]
  },
  "include": ["src", "vite.config.ts", "server.ts"]
...
- Shared formatting and lint rules SHOULD come from `@faasjs/dev/configs`.
...
import { oxfmtConfig, oxlintConfig } from '@faasjs/dev/configs'
...
- shared `fmt` and `lint` rules come from `@faasjs/dev/configs`
