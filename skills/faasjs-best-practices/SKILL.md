...
- **Data fetching**: `useFaas` for component-owned requests. `faas` for event handlers. the `Form` `faas` prop for form submits. `useFaasStream` for streaming. In apps using `@faasjs/ant-design`, import `faas`, `useFaas`, `useFaasStream`, `FaasReactClient`, `FaasDataWrapper`, and `withFaasData` from `@faasjs/ant-design` so error feedback stays consistent.
...
- **Return values**: return directly, avoid single-use intermediate variables. Destructure React hook returns and only the handler context in `defineApi`/`defineJob`; keep business input visible as `params.key`.
...
- Do not destructure function parameters except the first handler context parameter passed to `defineApi` or `defineJob` (e.g., `handler({ params })`, `handler({ params, event })`, or `handler({ params, logger, job })`). This exception is only for the top-level handler context; do not further destructure business fields from `params` such as `const { id } = params`. Read them as `params.id`, just like React components read `props.id`, so the source of each value is immediately visible. Access all other function, handler, component, hook, and callback parameters through the parameter object (e.g., `input.xxx`, `props.xxx`, or `data.xxx`).
